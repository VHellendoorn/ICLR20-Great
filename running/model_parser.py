	...
	def call(self, tokens, token_mask, edges, training):
		...
		for model in self.stack:
			if isinstance(model, rnn.RNN):  # RNNs simply use the states
				...
				states = model(states, mask, attention_bias, training=training)  # Note that plain transformers (with bias_dim=None on init) will simply ignore the attention_bias.
